...
排版前后指纹一致，我又用 python-docx 逐段比对了全文正文文本，也完全一致。同一次运行里，正文段落的字体从未设置变为宋体 12pt、首行缩进从无到 2 字符、对齐从无到两端对齐；摘要标题变为宋体 18pt 居中；中文关键词行变为宋体 12pt 左对齐——格式实实在在改了，正文一个字没动。同一份报告也如实报出了它发现的真问题：`char_below_min`（正文低于格式要求的最低字数）和 `blank_page_risk`（可能存在的强制分页空白页），不会因为指纹一致就假装一切完美。

> 保护范围：指纹比对前会归一化掉空白和项目符号字符，覆盖正文段落和表格；页眉页脚（本就是排版工具按规则主动设置的）不在比对范围内。
